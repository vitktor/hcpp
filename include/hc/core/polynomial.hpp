...
public:
  Polynomial(std::vector<T> coefficients, Eigen::MatrixXi exponents,
  ...
  const Eigen::MatrixXi& getExponents() const { return exps; }

  void setExponents(Eigen::MatrixXi exponents) { exps = std::move(exponents); }
