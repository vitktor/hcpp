...
// H(x,t) = (1-t)*start(x) + t*target(x). start and target must share the
// same variable list in the same order, since Hx_out columns are indexed
// by that shared order.
...
public:
               ...
               std::vector<std::complex<Scalar>>& H_out) const override
  {
    ...
    std::complex<Scalar> one_minus_t(Scalar(1) - t);
    std::complex<Scalar> tc(t);

    H_out.resize(F.size());
    for (size_t i = 0; i < F.size(); ++i)
      H_out[i] = one_minus_t * hc::evaluate(G[i], x) + tc * hc::evaluate(F[i], x);
  }
      ...
      std::vector<std::complex<Scalar>>& Ht_out) const override
  {
    ...
    for (size_t i = 0; i < n_eq; ++i)
    {
      ...
      H_out[i] = one_minus_t * g_val + tc * f_val;
      Ht_out[i] = f_val - g_val;
      for (size_t j = 0; j < n_vars; ++j)
        Hx_out[i][j] = one_minus_t * hc::evaluate(Gx[i][j], x) + tc * hc::evaluate(Fx[i][j], x);
    }
  }
